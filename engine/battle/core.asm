...
.enemyspeedboost
	...
	pop hl
	pop de
	jr nc, .noenemyweatherability
	call WeatherSpeedBoost

.noenemyweatherability
	ld c, 2
	call CompareBytes
	jr z, .speed_tie
	jp nc, .player_first
...
INCLUDE "engine/battle/ability_core.asm"
...
.got_anim
	...
	call GetBattleVar
	cp PSN
	jr nz, .normalstatuschecks
	ld a, BATTLE_VARS_ABILITY
	call GetBattleVar
	cp POISON_HEAL
	jr nz, .normalstatuschecks

	call HandlePoisonHeal
	jr .did_psn_brn
.normalstatuschecks
...
.battle_tower
	...
	;ld hl, wPayDayMoney		this might break battle tower, not sure
	;ld a, [hli]
	;or [hl]
	;inc hl
	;or [hl]
...
PlayVictoryMusic:
	...
	pop de
	jr nz, .play_music
	;ld hl, wPayDayMoney			this should be fine
	;ld a, [hli]
	;or [hl]
	;jr nz, .play_music
	ld a, [wBattleParticipantsNotFainted]
